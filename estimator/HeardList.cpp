...
BeaconInfo::BeaconInfo(int bid, Vector vLocation)
{
	this->bid = bid;
	reflectionCount = 0;
	vLocations.clear();
...
BeaconInfo::BeaconInfo(BeaconInfo *beaconInfo)
{
	this->bid = beaconInfo->bid;
	this->reflectionCount = beaconInfo->reflectionCount;
	this->vLocations = beaconInfo->vLocations;
	this->pPlanes = beaconInfo->pPlanes;
	this->parent = beaconInfo->parent;
	this->children = beaconInfo->children;

}

BeaconInfo BeaconInfo::ReflectByPlane(Plane *plane)
{
	BeaconInfo beaconInfo(this);
	reflectionCount ++;
	Vector vReflected = plane->GetReflectedPoint(vLocations.back());
	vLocations.push_back(vReflected);
	pPlanes.push_back(plane);

	return beaconInfo;
}

void BeaconInfo::AddChild(BeaconInfo* child)
{
